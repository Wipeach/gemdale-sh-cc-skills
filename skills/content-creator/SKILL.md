...
### 第五阶段：归档与上传
完成内容审核与修正后，执行以下步骤将成果固化并同步至飞书云盘。

1.  **本地保存**:
    *   将最终定稿的内容保存为 Markdown 文件。
    *   **命名规则**: 使用文章标题作为文件名（例如：`深度穿透_上海二手房市场的以价换量.md`）。
    *   **操作**: 直接在当前项目目录下创建文件。

2.  **上传飞书 (Feishu Drive)**:
    *   调用上传脚本（脚本位置 `scripts/upload_feishu.py` ）将本地 `.md` 文件同步至指定的飞书云文档文件夹。
    *   **指令**: `python scripts/upload_feishu.py <filename.md>`
    *   **验证**: 确认脚本返回 "Upload successful" 消息及文件 Token。